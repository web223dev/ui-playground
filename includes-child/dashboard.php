...
add_filter( 'login_headerurl', 'cb_custom_login_url', 10, 4 );
/**
 * Logo Backend Home link
 *
 * @since 1.0.0
 */
function cb_custom_login_url() {
	return site_url();
}

add_filter( 'login_headertext', 'cb_login_header_title' );
/**
 *  Logo Title Attribute uses Site Name
 *
 * @since 1.7.0
 */
function cb_login_header_title() {
	return get_bloginfo( 'name' );
}


add_action('after_setup_theme', 'cb_remove_admin_bar');
/**
 *  Remove admin bar for subscribers.
 *
 * @since 1.0.0
 */
function cb_remove_admin_bar() {
...
add_action( 'admin_head', 'cb_hide_tabs' );
/**
 *  Hide 'Screen Option' and 'Help' tab for non-Admins
 *
 * @since 1.0.0
 */
function cb_hide_tabs() {
...
add_action( 'wp_print_styles', 'cb_deregister_gbstyles', 100 );
//Disable gutenberg style in Frontend
function cb_deregister_gbstyles() {
