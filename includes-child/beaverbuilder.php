 ...
 */
 ...
 add_filter( 'fl_module_upload_regex', 'cb_upload_image_formats', 10, 4 );
/**
 * Add SVG to allowed regex for BB module uploads
 * @since 1.0.0
 */
 function cb_upload_image_formats( $regex, $type, $ext, $file ) {
    $regex[ 'photo' ] = '#(jpe?g|png|gif|bmp|tiff|svg?)#i';

    return $regex;
}

add_filter( 'image_size_names_choose', 'cb_custom_image_sizes' );
/**
* Helps Beaver Builder see custom sizes.
* @since 1.0.0
* @link https://kb.wpbeaverbuilder.com/article/382-add-custom-image-sizes
*/
function cb_custom_image_sizes( $sizes ) {
...
// add_filter( 'fl_builder_font_families_system', 'cb_added_webfonts' );
// add_filter( 'fl_theme_system_fonts', 'cb_added_webfonts' );
/**
 ...
 * @since 1.0.0
 */
function cb_added_webfonts( $system ) {
...
add_filter( 'fl_theme_breakpoint_opts', 'cb_custom_breakpoints');
/**
 * Return array of theme breakpoints for BB Theme
 * @since 1.7.3
 */
function cb_custom_breakpoints() {
...
add_filter( 'fl_builder_is_node_visible', 'cb_check_field_connections', 10, 2 );
/** 
 * Dont output empty custom field connections
 * @since 1.0.0
 */
function cb_check_field_connections( $is_visible, $node ) {
...
add_filter( 'fl_theme_builder_assets_expire', 'cb_builder_expire' );
/** 
 * Delay Themer Builder Assets to Expire for 10 years ~ default is 30 days
 * @since 1.0.0
 */
function cb_builder_expire() {
...
add_filter( 'fl_builder_color_presets', 'cb_builder_color_presets' );
/** 
 * Add color presets for Beaver Builder
 * @since 1.0.0
 */
function cb_builder_color_presets( $colors ) {
    $colors = array();
...
add_action( 'customize_controls_print_footer_scripts', 'cb_customize_color_presets' );
/** 
 * Add color presets for Beaver Builder Customizer Theme
 * @since 1.0.0
 */
function cb_customize_color_presets() {
    ?>
  <script>
